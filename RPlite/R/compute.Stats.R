...
function (null.distrib, final.scores, nb.genes, nperms) 
{
    ...
    results <- data.frame(rows = 1:nb.genes, scores = final.scores, 
        rank.scores = revert.ind, pvals.up = pvals.up[revert.ind], 
        qvals.up = qvals.up[revert.ind], pvals.do = pvals.do[revert.ind], 
